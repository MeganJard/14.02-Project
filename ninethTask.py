...
class InputBoxforadress(InputBox):
    def __init__(self, x, y, w, h, text='', search_flag=True, crossy_flag=True):
        ...
        self.searchImg = pygame.image.load('data/postButton.png')
        self.hoveredsearchImg = pygame.image.load('data/hoveredPostButton.png')
        self.pressedsearch = False
...
adress = InputBoxforadress(10, 350, 300, 35, search_flag=True, crossy_flag=False)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            elif adress.hover_on_search(event.pos):
                adress.pressedsearch = not adress.pressedsearch
                if adress.pressedsearch:
                    ...
                    try:
                        txt_adr = response.json()["response"]["GeoObjectCollectio"
                                  "n"]["featureMember"][0]["GeoObject"]["metaDataP"
                                                                        "roperty"][
          "GeocoderMetaData"]["Address"]['postal_code']
                        adress.text_li.append((SFONT.render(txt_adr, True, adress.color), (adress.rect.x + 5, adress.rect.y + 2 + 9 * 2)))
                    except Exception:
                        adress.text_li.append((SFONT.render('No postal code', True, adress.color), (adress.rect.x + 5, adress.rect.y + 2 + 9 * 2)))
                else:
                    del adress.text_li[-1]
